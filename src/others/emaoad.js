...
(function(document, window, undefined) {
    ...
    var cat = {//页面广告初始化投放广告
        ...
        shouldReport: function() {
            return !!((this.external.provinceId || tool.cookie.get('province_id') || this.remote.provinceId) && this.usedCityId);
        },
        ...
        wrapThenRenderThenReport: function(resource, divId, zid, cityId) { //可能异步，也可能同步。取决于是否能从外部参数以及本地cookie中读取到城市id，对广告的渲染必须得到这个值后才能进行。如果没有，发起异步请求成功后再执行
            ...
            var render = tool.bind(cat.render, cat, catFood.get('data'), catFood.get('tpl'), divId); //插入到文档
            var request = tool.bind(cat.request, cat, catFood.get('impurl'), catFood.get('impurl3'), catFood.get('aid'), zid, divId); //向后台报告统计数据

            cat.preload(catFood.get('img'), render, this.asyncOrSyncReport(request)); //先把广告图片加载下来再render，再report;
            return true;
        },
        render: function(data, tpl, target) { // 将广告插入文档
            if (!data) return;
        ...
        request: function(url, url3, aid, zid, divId,city_id) { // zid，divId，aid绑定到每次请求上，而每次请求的负载的各种统计参数，则由统计对象负责
            var img = new Image();
            var strategy = function(item) {
                return item(city_id);
            };
            var search = '?zid=' + zid + '&aid=' + aid + '&' + tool.map(statisticalStrategies, strategy).join('&') + '&t=' + (Math.random() + '').replace('.', '') + (new Date().getTime());
    ...
    function update(cityId,zid,ziddata){//不再放入对象里了，为了压缩更加彻底
        ...
        if(!catFood.data)return;
        var render = tool.bind(cat.render, cat, catFood.get('data'), catFood.get('tpl'),divId); //插入到文档
        var request = tool.bind(cat.request, cat, catFood.get('impurl'), catFood.get('impurl3'), catFood.get('aid'), zid, divId,cityId); //向后台报告统计数据
